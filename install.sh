...
install_vray-panel() {
    ...
    if [ $# == 0 ]; then
        ...
        curl --L --insecure -o /usr/local/vray-panel-linux-$(arch).tar.gz https://github.com/IlyadKruger/vray-panel/releases/download/${last_version}/vray-panel-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        curl --L --insecure -o /usr/local/vray-panel-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
    ...
    curl --L --insecure -o /usr/bin/vray-panel https://raw.githubusercontent.com/IlyadKruger/vray-panel/main/vray-panel.sh
