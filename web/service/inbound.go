...
import (
	...
	"vray-panel/database"
	"vray-panel/database/model"
	"vray-panel/logger"
	"vray-panel/util/common"
	"vray-panel/xray"
...
func (s *InboundService) ResetClientTraffic(clientEmail string) (bool, error) {
    needRestart := false
    var remarks []string
    db := database.GetDB()
    if err := db.Model(model.Inbound{}).Pluck("remark", &remarks).Error; err != nil {
        logger.Warning(err)
        return false, err
    }
    for iteration, remark := range remarks {
	iterationNumber := iteration + 1
        modifiedEmail := clientEmail + "(" + remark + ")"
        traffic, err := s.GetClientTrafficByEmail(modifiedEmail)
        if err != nil {
            return false, err
        }
        if !traffic.Enable {
            inbound, err := s.GetInbound(iterationNumber)
            if err != nil {
                return false, err
            }
            clients, err := s.GetClients(inbound)
            ...
            for _, client := range clients {
                if client.Email == modifiedEmail {
                    ...
                    if err1 == nil {
                        logger.Debug("Client enabled due to reset traffic:", modifiedEmail)
                    } else {
...
func (s *InboundService) GetClientTrafficByEmailEdited(email string) (*xray.ClientTraffic, error) {
    db := database.GetDB()
    var traffic1 *xray.ClientTraffic

    // Fetch remark values from the database
    var remarks []string
    if err := db.Model(model.Inbound{}).Pluck("remark", &remarks).Error; err != nil {
        logger.Warning(err)
        return nil, err
    ...
    // Loop through remark values
    for i, remark := range remarks {
        // Modify email based on remark
        modifiedEmail := email + "(" + remark + ")"

        // Fetch client traffic data for the modified email
        var traffic xray.ClientTraffic
        if err := db.Model(xray.ClientTraffic{}).Where("email = ?", modifiedEmail).First(&traffic).Error; err != nil {
            logger.Warning(err)
            return nil, err
        }

        // Add the values of Up and Down to traffic1
        if i == 0 {
            traffic1 = &traffic
        } else {
            traffic1.Up += traffic.Up
            traffic1.Down += traffic.Down
        }
    }

    // Modify the email field of traffic1
    if traffic1 != nil {
        traffic1.Email = email
    } else {
